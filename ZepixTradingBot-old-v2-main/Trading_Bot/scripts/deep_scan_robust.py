...
print("🔍 STARTING DEEP SYNTAX ANALYSIS (LATIN-1 ENCODING)...")
print("="*60)
...
for file_path in files_to_check:
    try:
        ...
        # Try reading as binary first to detect null bytes
        with open(file_path, "rb") as f:
            content = f.read()
            if b'\x00' in content:
                print(f"❌ NULL BYTES DETECTED in {file_path} - File corrupted!")
                error_count += 1
                continue
                
        # Try reading as UTF-8 with replacement for safer parsing checks
