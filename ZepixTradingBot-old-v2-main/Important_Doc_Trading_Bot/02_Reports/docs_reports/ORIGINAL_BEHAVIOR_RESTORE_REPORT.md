# ORIGINAL BEHAVIOR RESTORE REPORT
...
Successfully restored original bot behavior where `tf` field is **REQUIRED** (not optional) for all signals. This ensures all 3 logics (LOGIC1, LOGIC2, LOGIC3) work correctly.
...
```python
# Before:
tf: Optional[str] = "5m"  # Optional with default

# After:
tf: str  # REQUIRED FIELD (no default)
```
